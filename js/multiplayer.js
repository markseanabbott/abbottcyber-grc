...
function mpPickRole(rid) {
  ...
  if (btn && role) { btn.disabled=false; btn.textContent=`Join as ${role.name} â†’`; }
}
...
async function mpDoJoin(code) {
  ...
  if (!session)        { if(errEl) errEl.textContent='Session data missing â€” refresh and try again.'; return; }
  ...
  try {
    ...
    if (existing.find(p=>p.role_id===mpSelectedRole)) {
      ...
      if(errEl) errEl.textContent='That role was just taken â€” pick another.';
  ...
  if (!session.declaration_logged) {
    ...
    wrap.innerHTML=mpScreenHTML(`
      ...
      <div style="text-align:center;margin-top:1rem">
        ...
        <div style="font-size:12px;color:rgba(255,255,255,0.4)">${isIC?'Or wait for the facilitator to begin from their screen.':'Polling for exercise startâ€¦'}</div></div>
...
async function mpRenderInjectView(wrap, session, playerState) {
  ...
  const cardGrid = cards ? `
    <div class="mp-label" style="margin-bottom:8px">Choose your action â€” tap a card to learn more</div>
      ...
      <div class="mp-couter${savedCardIdx===i?' sel-card':''}" id="mpCard_${i}" data-v="${card.v.replace(/"/g,'&quot;')}" onclick="mpFlipCard(${i})">
        <div class="mp-cinner">
          <div class="mp-cface">
            ...
            <div class="mp-ctapehint">Tap for details â†©</div>
          </div>
          <div class="mp-cbk">
            <div>
    ...
    <div class="mp-custom-link" onclick="mpPlayCustom()">or write a custom response instead â†“</div>
  ...
  wrap.innerHTML=mpScreenHTML(`
    ...
    <div class="mp-card">
      ...
      <div id="mpRespMsg" style="text-align:center;font-size:11px;color:rgba(255,255,255,0.45);margin-top:6px;min-height:14px">
        ${savedResp?'Response saved â€” waiting for facilitator to advance.':''}</div>
    </div>`);
...
async function mpSubmitResponse(sessionId, injectIdx, roleId, playerName) {
  ...
  try {
    ...
    if(msgEl) msgEl.textContent='Response saved â€” waiting for facilitator to advance.';
  } catch(e) {
...
// ============================================================
// MULTIPLAYER â€” Display screen (?display=CODE)
// ============================================================
  ...
  try {
    ...
    listEl.innerHTML=TT_ROLES.map(role=>{
      ...
      return `<div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid rgba(255,255,255,0.07)">
        ...
        ${p?'<span style="font-size:10px;font-weight:700;color:#86efac;background:rgba(21,128,61,0.3);padding:2px 8px;border-radius:99px">JOINED</span>':'<span style="font-size:10px;color:rgba(255,255,255,0.25)">â€”</span>'}
      </div>`;
    }).join('');
  } catch {}
}
...
async function mpIcStartExercise(sessionId) {
  ...
  try {
    ...
    if(btn){btn.textContent='âœ“ Exercise started â€” facilitator will advance shortlyâ€¦';}
  } catch(e) {
