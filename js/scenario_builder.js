...
function renderScenarioBuilder() {
  return sbState.view === 'editor' ? renderSbEditor() : renderSbList();
}
      ...
      <tbody>
        ${builtins.map(s => `
          <tr>
            ...
            <td style="text-align:right"><button class="btn btn-outline btn-sm" onclick="sbCloneBuiltin('${s.id}')">Clone →</button></td>
          </tr>
        `).join('')}
      </tbody>
    </table>
  ...
  <div class="sb-inject-row" id="sb-inj-row-${idx}">
    <div class="sb-inject-header" onclick="sbToggleInject(${idx})">
      ...
      <div style="display:flex;gap:6px;align-items:center;flex-shrink:0" onclick="event.stopPropagation()">
        ...
        <span style="color:var(--muted);font-size:14px;cursor:pointer">${isOpen ? '▲' : '▼'}</span>
      </div>
    </div>
